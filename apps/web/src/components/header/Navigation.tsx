...
import { Menu, X, Sun, Moon, Command, Blend, KeyRound, ChevronDown, User } from 'lucide-react';
...
const Navigation = () => {
	...
	const isToolActive = () => toolsItems.some((item) => location.pathname === item.path);
		...
		<nav className='navigation'>
			<div className='navigation__container'>
				...
				<div className='navigation__desktop-menu'>
							...
							{index === menuItems.length - 2 && (
								<div className='navigation__tools-dropdown'>
									<button
										...
										<span>Tools</span>
										<ChevronDown className={`icon ${isToolsOpen ? 'rotate' : ''}`} />
									</button>
					...
					<button type='button' onClick={toggleTheme} className='navigation__theme-toggle' aria-label='Toggle theme'>
						{getThemeIcon()}
					</button>
				...
				<div className='navigation__mobile-menu'>
					...
					<Link className='navigation__theme-toggle' aria-label='Toggle theme' to={'/me'}>
						<User width={24} height={24} />
					</Link>

					<button type='button' onClick={toggleMenu} className='navigation__mobile-menu-button' aria-expanded={isOpen}>
						{isOpen ? <X className='icon' aria-hidden='true' /> : <Menu className='icon' aria-hidden='true' />}
					</button>
				</div>
			</div>
											...
											{toolsItems.map((item) => (
												<Link
													...
													className={`navigation__dropdown-link navigation__dropdown-link--indented ${
														isActive(item.path) ? 'navigation__dropdown-link--active' : ''
													}`}>
													{item.icon}
													<span>{item.name}</span>
												</Link>
