...
func (h *HealthCheckController) addServiceStatus(
	serviceList *[]types.HealthCheck, name string, isUp bool, message *string,
) {
	status := "Up"
	...
	*serviceList = append(*serviceList, types.HealthCheck{
		Name:    name,
		Status:  status,
		IsUp:    isUp,
		Message: message,
...
func (h *HealthCheckController) Check(c *fiber.Ctx) error {
	isHealthy := true
	var serviceList []types.HealthCheck
	...
	return c.Status(statusCode).JSON(types.HealthCheckResponse{
		Status:    status,
