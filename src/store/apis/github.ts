...
const ISSUES_REPOSITORIES = [
	'samiurprapon/aether',
	'samiurprapon/personal-blog',
];
	...
	endpoints: (builder) => ({
		...
		>({
			...
			transformResponse: (response: ApiResponse | ApiErrorResponse) => {
				if ('error' in response) {
					throw new Error(
						`Fetching GitHub contribution data failed: ${response.error}`,
					);
				}
		...
		>({
			...
			transformResponse: (response: ApiResponse, _meta, arg) => ({
				total:
					response.total[
						arg.year === 'last' ? 'lastYear' : arg.year || 'lastYear'
					],
		...
		getGithubIssues: builder.query<GithubIssue[], void>({
			async queryFn(_arg, _queryApi, _extraOptions, baseQuery) {
				try {
					...
					const filteredIssues = uniques.filter(
						(issue) => !issue.labels.some((label) => label.name === 'blog'),
					);

					// Cache results
					localStorage.setItem(
						ISSUES_CACHE_KEY,
						JSON.stringify({ data: filteredIssues, timestamp: Date.now() }),
					);
